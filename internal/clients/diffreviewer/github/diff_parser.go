...
func (p *multiFileParser) parse() ([]*diffreviewer.FileDiff, error) {
	var fds []*diffreviewer.FileDiff
	fp := &fileParser{r: p.r}
	for {
		fd, err := fp.parse()
		if err != nil || fd == nil {
...
func (p *fileParser) parse() (*diffreviewer.FileDiff, error) {
	fd := &diffreviewer.FileDiff{}
...
func (p *fileParser) parseHunks() ([]*diffreviewer.Hunk, error) {
	b, err := p.r.Peek(len(tokenOldFile))
	if err != nil {
	...
	if !bytes.HasPrefix(b, []byte(tokenStartHunk)) {
		...
		if bytes.HasPrefix(b, []byte(tokenDiffGit)) {
			...
			return []*diffreviewer.Hunk{}, nil
		}
		return nil, ErrNoHunks
	}
	var hunks []*diffreviewer.Hunk
	hp := &hunkParser{r: p.r}
	for {
		h, err := hp.parse()
		if err != nil {
...
func (p *hunkParser) parse() (*diffreviewer.Hunk, error) {
...
endhunk:
	for !p.done(lold, lnew, hr) {
		...
		case tokenUnchangedLine, tokenAddedLine, tokenDeletedLine:
			p.lnumdiff++
			l, _ := readline(p.r)
			line := &diffreviewer.Line{Content: l[len(token):]} // trim first token
			switch token {
			case tokenUnchangedLine:
				line.Type = diffreviewer.LineUnchanged
				...
				lnew++
			case tokenAddedLine:
				line.Type = diffreviewer.LineAdded
				line.LnumDiff = p.lnumdiff
				line.LnumNew = lnew
				lnew++
			case tokenDeletedLine:
				line.Type = diffreviewer.LineDeleted
				line.LnumDiff = p.lnumdiff
				line.LnumOld = lold
				lold++
			}
